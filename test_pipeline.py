        ...
        for node_name, state in output.items():
            ...
            if "simulation_results" in state and node_name == "simulate":
                for res in state["simulation_results"]:
                    print(
                        f"Variant {res.get('variant_id')} CTR: {res.get('ctr') * 100:.2f}% (Opens: {res.get('open_rate') * 100:.2f}%)"
                    )

            if node_name == "evaluate":
                if state.get("is_winner_determined"):
                    print(
                        f"WINNER DECIDED! Variant {state['winner']['variant_id']} won with {state['winner_confidence'] * 100:.2f}% confidence."
                    )
                else:
